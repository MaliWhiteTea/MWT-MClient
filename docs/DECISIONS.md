...
| D-057  | Windows ACL yardımcısı .NET 10 LTS ile yalnız `win-x64` için self-contained tek dosya üretilir. Kararlı SDK'nın `10.0.4xx` feature band'i sabitlenir; trimming ve Native AOT kapalıdır. Build ve gerçek ACL entegrasyon testleri Windows x64 ortamında çalışır; çıktı checksum'ı MSI paket manifestinde doğrulanır. | Kabul edildi |
