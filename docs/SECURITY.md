...
- Linux/Raspberry Pi adaptörü root olmayan servis UID'si sahipliğinde dizin için tam `0700`, korumalı dosya için tam `0600` ister; UID `0`'ı, grup/diğer erişimini, sembolik bağı ve beklenmeyen dosya türünü reddeder.
...
- Yardımcı program .NET 10 LTS ile `win-x64`, self-contained ve tek dosya yayımlanır. Trimming ve Native AOT, güvenlik API'lerinde sessiz davranış değişikliği veya platforma özgü derleme karmaşıklığı oluşturmaması için ilk sürümde kapalıdır. Build yalnız kararlı .NET 10 SDK'sıyla Windows x64 üzerinde yapılır; yayımlanan dosyanın checksum'ı paket manifestine girer ve MSI üretiminden önce doğrulanır.
